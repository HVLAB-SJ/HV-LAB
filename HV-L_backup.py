...
class ProjectComboBox(QComboBox):
    ...
    def paintEvent(self, event):
        ...
        if text == "?„ë¡œ?íŠ¸ ê´€ë¦?:
            ...
            top_y = text_rect.center().y() - line_spacing // 2 - text1_height // 2 + 3  # 3px ?„ë˜ë¡?
            ...
            if unit_info:
                ...
                bottom_y = text_rect.center().y() + line_spacing // 2 + text2_height // 2 + 3  # 3px ?„ë˜ë¡?
...
class CustomDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if text in ["?„ë¡œ?íŠ¸ ê´€ë¦?, "ê³µì • ê´€ë¦?]:
            ...
            if text == "?„ë¡œ?íŠ¸ ê´€ë¦?:
                font = QFont("ë§‘ì? ê³ ë”•", 12)  # ?„ë¡œ?íŠ¸ ê´€ë¦?12pxë¡?ë³€ê²?
            else:
                font = QFont("ë§‘ì? ê³ ë”•", 9)  # ê³µì • ê´€ë¦¬ëŠ” ê·¸ë?ë¡?
            painter.setFont(font)
...
class InteriorSettlementApp(QMainWindow):
    ...
    def create_project_section(self):
        ...
        self.project_combo.setMinimumHeight(95)  # 100?ì„œ 95ë¡?ë³€ê²?
        self.project_combo.setMaximumHeight(95)  # ìµœë? ?’ì´??95ë¡??¤ì •
    ...
    def create_input_section(self):
        ...
        # ??ª©ëª?
        input_layout.addWidget(QLabel("??ª©ëª?"), 2, 0)
    ...
    def create_table_section(self):
        ...
        self.table.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)  # ?¸ë¡œë¡??•ì¥ ê°€??
        ...
        for text, func, style in buttons:
            ...
            if key == "?? œ":
                ...
                """)
                self.sync_status_label.setFixedSize(10, 10)  # ê³ ì • ?¬ê¸°ë¥?ë°˜ìœ¼ë¡?
                self.sync_status_label.setToolTip("?¤ì‹œê°??™ê¸°??ì¤?)
            ...
            try:
                ...
                for filename in os.listdir(backup_dir):
                    if filename.startswith("auto_backup_") and filename.endswith(".json"):
                        ...
                        if current_time - file_time > 7 * 24 * 60 * 60:  # 7??
                            try:
                                os.remove(file_path)
                            except:
                                pass
    ...
    def show_update_dialog(self, version, download_url):
        ...
        cancel_btn.setStyleSheet(GRAY_BUTTON_STYLE.replace("12px", "24px").replace("32px", "64px"))  # ?°íŠ¸?€ ?’ì´ 2ë°?
        cancel_btn.setMinimumHeight(64)
        cancel_btn.clicked.connect(dialog.reject)
        
        ok_btn = QPushButton("ì§€ê¸??…ë°?´íŠ¸")
        ok_btn.setStyleSheet(BUTTON_STYLE.replace("12px", "24px").replace("32px", "64px"))  # ?°íŠ¸?€ ?’ì´ 2ë°?
